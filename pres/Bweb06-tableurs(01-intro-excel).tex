...
\begin{itemize}
	...
	\end{itemize}
	\item Référence d'une plage : on peut référencer une plage de cellule ; en générale, on la référence de cellule de départ suivie par \keyword{:} suivie par le référence de cellule de destination (eg. \expword{C2:D6})
\end{itemize}
\end{frame}
...
\begin{itemize}
	\item \optword{SOMME} : calcule la somme entre plusieurs valeurs (eg. \expword{SOMME(E3;E6;2)}) ou une plage de cellules (eg. \expword{SOMME(E3:E6)})
...
\begin{minipage}{0.81\textwidth}
	\begin{itemize}
		...
		\item \optword{RACINE} : donne la racine carrée d'un nombre. Si le nombre est négatif, elle rend une erreur \keyword{\#NOMBRE!}
...
\begin{minipage}{0.69\textwidth}
	\begin{itemize}
		\item \optword{SI} : permet d'établir des comparaisons logiques entre une valeur et le résultat attendu. Elle rend le deuxième argument si la condition est vrai ; le troisième argument sinon.
...
\begin{minipage}{0.69\textwidth}
	\begin{itemize}
		...
		\item \optword{AUJOURDHUI} : rend la date d'aujourd'hui
		\item \optword{ANNEE} : rend l'année à partir d'une date
	\end{itemize}
\end{minipage}
%
\begin{minipage}{0.3\textwidth} 
...
\begin{minipage}{0.69\textwidth}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item elle rend la valeur correspondante dans la colonne \expword{2} ; c-à-d, dans la plage \expword{F2:F4}
		\end{itemize} 
